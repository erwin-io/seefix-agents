...
agent = FacilityInspectionAgent(settings, provider=provider)
...
def analyze_report(
    ...
    if image is None:
        raise gr.Error("Select a JPEG, PNG, or WebP image first.")

    try:
        result = agent.analyze(image_to_bytes(image))
        return result.model_dump(mode="json")
    except (ProviderError, ValueError) as exc:
        raise gr.Error(str(exc)) from exc


with gr.Blocks(title="SEEFIX Facility Inspection Agent") as demo:
    gr.Markdown(
        """
    ...
    with gr.Row():
        ...
        with gr.Column():
            result_output = gr.JSON(label="Structured assessment")
